...
const port = "3000";
const usersAddress = "http://localhost:"+port+"/users";
const companiesAddress = "http://localhost:"+port+"/companies"
...
function fetchCompany() {
    fetch(companiesAddress)
        ...
        .then(()=>{
            ConnectUsersToCompany();
        })

        .then(()=>{
            SortCompaniesByUsers();
...
function ConnectUsersToCompany() {
    for (let i = 0; i < usersList.length; i++) {
        let companyNumber = usersList[i].companyNumber;
        let companyUserName = usersList[i].name;
        companiesList[companyNumber].users.push(companyUserName);
    }
}

//Sortujemy firmy po przez uzytkownikow
...
function insertUsersInTable(i) {
    for (let j = 0; j < companiesList[i].users.length; j++) {
        let index = companiesList[i].users[j].slice(5)
